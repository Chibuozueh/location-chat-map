...
/**
 * LSI × Lewis Scholars Imhotep Project partnership lockup.
 *
 * Three stacked layers make up the mark:
 *   - Gilded yellow sun-burst + arms-raised stickman (figure)
 *   - Crimson red banner with notched ends (ribbon)
 *   - Black marquee block (theatrical-sign silhouette)
 *
 * The "LSI" type renders OUTSIDE the SVG so it can scale cleanly at any
 * size. The full partnership wordmark — "Morehouse College / Lewis
 * Scholars / Imhotep Project" — sits beside the figure in a typographic
 * lockup using the Atlanta Atlas display + sans fonts. When the lockup
 * sits beside the primary Morehouse app logo, pass `hideInstitution` to
 * drop the redundant "Morehouse College" line.
 *
 * Colors pinned to:
 *   - Gilded yellow  → #F5C518  (figure / accent)
 *   - Crimson red    → #C8242E  (ribbon / banner)
 *   - Marquee black  → #111111  (sign silhouette / type)
 *   - Morehouse maroon → #6E0E1E (wordmark accent)
 *   - Parchment      → #FAF7F2  (outline / sign panel)
 */
...
const containerBySize: Record<
  LockupSize,
  { box: string; figure: number; college: string; scholars: string; project: string }
> = {
  sm: {
    box: "h-12",
    figure: 38,
...
export function LsiMark({
  ...
  // from chip-sizes up to large-format lockups.
  return (
    <svg
      viewBox="0 0 120 140"
    ...
    >
      ...
      {/* Stickman body — black underlay gives the figure its silhouette on
          light card backgrounds, then the gilded yellow stroke is layered
          on top with paintOrder="stroke" so colors stay vivid. */}
      <g
        stroke="#111111"
        strokeWidth="6.6"
        strokeLinecap="round"
      ...
      <g>
        {/* Subtle drop-shadow strip for depth. */}
        <polygon
      ...
      {/* Marquee block — bold black rounded rect. Type is rendered in the
          React wordmark beside the SVG so it can scale to any size. */}
      <g>
        <rect
          x="22"
          y="108"
...
export function LsiLogo({
  ...
  return (
    ...
    >
      <LsiMark size={cfg.figure} className="-mt-0.5" />
      {withWordmark && (
